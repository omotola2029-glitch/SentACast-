...
# Custom CSS with green/blue color scheme and button height fix
st.markdown("""
<style>
...
def generate_pdf_report(df, metrics, forecast_data=None, sentiment_chart=None, intent_chart=None):
    ...
    summary_text = f"""
    This report presents a comprehensive sentiment analysis of {metrics['total_reviews']:,} customer reviews.
    ...
    if 'positive_count' in metrics:
        ...
        # Add sentiment chart if provided
        if sentiment_chart:
...
def generate_word_report(df, metrics, forecast_data=None, sentiment_chart=None, intent_chart=None):
    ...
    doc.add_paragraph(
        f"This report presents a comprehensive sentiment analysis of {metrics['total_reviews']:,} customer reviews. "
...
if stage == "Home":
    ...
    st.write("""
    SentACast provides comprehensive sentiment analysis capabilities including automatic data cleaning and preprocessing, 
    ...
    motivations, human-in-the-loop correction mechanisms for improved accuracy, Sentiment trend forecasting usinG Prophet for 
    predictive insights, comprehensive metrics and visualizations for detailed analysis, and multi-format export capabilities 
    ...
    st.write("""
    To begin your analysis, prepare a CSV file containing customer reviews. Your file should include a column with review text 
    such as review_text, review body, or similar naming conventions. Optionally include a date column for time-based forecasting 
    and a rating column for additional insights. The system automatically detects and processes these columns regardless of naming variations.
    """)
...
2024-01-17,Absolutely love it! Highly recommend,5
    ...
    if uploaded_file or st.session_state.get('raw_df') is not None:
        try:
            ...
            if st.button("Clean and Preprocess Data", type="primary", key="clean_data") or st.session_state.get('data_cleaned'):
    ...
    if st.button("Run VADER Sentiment Analysis", type="primary", key="run_vader") or st.session_state.get('data_analyzed'):
            ...
            with st.spinner("Analyzing sentiments..."):
                ...
                if st.session_state['hitl_enabled']:
                    def flag_for_review(row):
                        ...
                        sarcasm_words = ['yeah right', 'sure', 'obviously', 'totally']
        ...
        st.subheader("Flagged Reviews Analysis")
    ...
    st.header("Human-in-the-Loop Review and Correction")
    ...
    st.header("Sentiment Trend Forecasting with Prophet")
    ...
    if 'sentiment' in df.columns:
        ...
        try:
            ...
            pass  # If plotly image export fails, continue without charts
        
        # Create intent pie chart
