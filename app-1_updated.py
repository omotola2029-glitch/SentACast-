...
def go_to_stage(stage_name):
    st.session_state['stage_override'] = stage_name
    st.rerun()
...
with col2:
    st.title("SentACast")
    st.caption("Sentiment Analysis and Forecasting System with Human Review")

st.divider()

# Sidebar nav
st.sidebar.header("Navigation")
if 'stage_override' in st.session_state:
    stage = st.session_state.pop('stage_override')
else:
...
2024-01-17,Absolutely love it! Highly recommend,5
    ...
    if uploaded_file or st.session_state.get('raw_df') is not None:
        try:
            ...
            if st.button("Clean & Preprocess Data", type="primary", key="clean_data") or st.session_state.get('data_cleaned'):
                ...
                if st.button("➡ Proceed to Sentiment Analysis", use_container_width=True):
    go_to_stage("Analyze Sentiment")
    ...
    if st.button("Run Sentiment Analysis", type="primary", key="run_vader") or st.session_state.get('data_analyzed'):
     ...
     # calc confidence scores
        ...
        if st.button("➡ Proceed to Review & Correction", use_container_width=True):
    go_to_stage("Review and Correction")
    ...
    if len(flagged_df) == 0:
        ...
        st.markdown("---")
        if st.button("➡ Proceed to Forecast Trends", use_container_width=True):
    go_to_stage("Forecast Trends")
    else:
